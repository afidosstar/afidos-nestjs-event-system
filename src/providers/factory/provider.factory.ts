  ...
  createProvider(
    ...
    try {
      switch (config.driver) {
        case 'smtp':
          return new SmtpEmailProvider(config.config);
        
        case 'twilio':
          return new TwilioSmsProvider(config.config);
        
        case 'http':
          return new HttpWebhookProvider(config.config);
        
        case 'firebase':
          return new FirebasePushProvider(config.config);
        
        case 'firebase-like':
          return new FirebaseLikeProvider(config.config);
